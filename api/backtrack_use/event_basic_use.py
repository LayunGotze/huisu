...
target={'time':['周一','周二','周三','周四','周五','周六','周日','实验'],
    'legend':['Verbal Cooperation','Material Cooperation','Verbal Conflict','Material Conflict'],
    'data':[{'name':'Verbal Cooperation','type':'line','smooth':'true','data':[10, 12, 21, 54, 260, 830, 710, 9]}]}
origin={'hot': [[1522339200000, 55], [1522425600000, 0], [1522512000000, 0], [1522598400000, 0], [1522684800000, 0], [1522771200000, 0], [1522857600000, 0], 
[1522944000000, 0], [1523030400000, 0], [1523116800000, 0], [1523203200000, 0], [1523289600000, 0], [1523376000000, 0], [1523462400000, 0]],'max_value': 55, 'min_value': 0}

def one_hot2target(origin):
    #将一种类型的事件热度图转换为echarts适合的类型
    data={'time':[],'legend':[],'data':[]}
    tmp_dict={'name':'','type':'line','smooth':'true','data':[]}
    data['legend']=['事件热度']
    data['data'].append(tmp_dict)
    data['data'][0]['name']=['事件热度']
    for item in origin['hot']:
        data['time'].append(time.strftime("%Y%m%d", time.localtime(item[0]/1000)))
        data['data'][0]['data'].append(item[1])
    return data

#print(one_hot2target(origin))
origin={'官方声明': {'hot': [[1522512000000, 2], [1522598400000, 34], [1522684800000, 38], [1522771200000, 51], [1522857600000, 21], [1522944000000, 12], [1523289600000, 1], [1523980800000, 1], [1524067200000, 14], [1524153600000, 13], [1524240000000, 5], [1524326400000, 4], [1524412800000, 15], [1524499200000, 14], [1524585600000, 38], [1524672000000, 6], [1524758400000, 10], [1524844800000, 9], [1524931200000, 12], [1525017600000, 7]], 'max_value': 51, 'min_value': 0},
...
def event_all_4_conclusion(origin):
    #将20种事件类型的统计整合为4个大类的统计
    ret={'time':[],'legend': ['口头合作', '实际合作', '口头冲突', '实际冲突'], 'data':[]}
    tmp_dict={'name':'','type':'line','smooth':'true','data':[]}
    for i in range(1,21):
        if i in origin:
            ret['time']=origin[i]['time']
            break
    if ret['time']==[]:
        return ret
    time_length=len(ret['time'])
    for i in range(1,5):
        ret['data'].append({'name':ret['legend'][i-1],'type':'line','smooth':'true','data':[0]*time_length})
    for i in range(0,20):
        if i+1 in origin:
            for j in range(0,time_length):
                ret['data'][(int)(i/5)]['data'][j]+=origin[i+1]['data'][0]['data'][j]
    return ret
...
# print(event_all_4_conclusion(origin))